...
export async function POST(req: Request) {
  try {
    ...
    // Captura o nome de quem está falando (Celio ou sua esposa no futuro)
    ...
    if (messageText.startsWith('/resumo')) {
      ...
      const summaryPrompt = `Resuma as últimas 24h para o ${userFirstName}. DADOS LOCAIS: \n${activityData}\n DADOS GOOGLE: \n${googleContext}`;
    ...
    // --- 4. BUSCA HD (Memórias Consolidadas) ---
    ...
    const ramMemory = history?.reverse().map(h => `${h.metadata?.user || 'User'}: ${h.content}\nJarvis: ${h.metadata?.ai_reply}`).join('\n') || "RAM Vazia.";

    // --- 6. SELEÇÃO DO MOTOR DE IA ---
    ...
    // --- 7. O PROMPT MESTRE DO JARVIS ---
    const dataAtual = new Date().toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo' });
    const finalPrompt = `
      USUÁRIO ATUAL: ${userFirstName}
    ...
    // --- 8. INTERCEPTADORES (Os Olhos e o Braço Mecânico) ---
    ...
    // Interceptador de Agendamento (GOOGLE AGENDA)
    ...
    await supabase.from('brain').insert([{
      ...
      metadata: { ai_reply: aiReply, context: contextTag, module: moduleTag, user: userFirstName }
    }]);

    await sendTelegram(chatId, aiReply);

    // --- 10. GATILHO DE APRENDIZADO SILENCIOSO (DESATIVADO TEMPORARIAMENTE) ---
    // COMENTADO para evitar o loop de mensagens. Limparemos o HD manualmente à tarde.
    // if (projectTag) {
...
async function createGoogleEvent(summary: string, startTime: string) {
  try {
    ...
    if (!startIso.includes('-') && !startIso.endsWith('Z')) startIso += '-03:00'; // Fuso de Brasília cravado
...
async function getGoogleContext() {
  try {
    ...
    // Lendo o Gmail (Pegando Assuntos reais)
    ...
    return await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { 
            role: "system", 
            content: "Você é o Jarvis. Personalidade: Empático, focado e inteligente. Adapte-se ao contexto: vibre com vitórias, seja prestativo em erros técnicos e acolhedor com assuntos familiares. SEJA SUCINTO, não enrole ou crie textos gigantes. DIRETRIZ CRÍTICA: Se precisar consultar e-mails ou agenda, responda APENAS com a tag exata: '[LER_CONTEXTO]'. Se o usuário pedir para agendar algo, use a tag: '[AGENDAR: Titulo | YYYY-MM-DDTHH:mm:ss]'. Não narre o que vai fazer, apenas execute ou responda com naturalidade." 
          }, 
...
// Mantive a função aqui embaixo intocada para podermos chamar ela na Rota Admin mais tarde
async function consolidateKnowledge(projectTag: string) {
  try {
    const { data: logs, count } = await supabase.from('brain').select('id, content', { count: 'exact' }).eq('project_tag', projectTag).is('metadata->consolidated', null).limit(6);
    if (count && count >= 5 && logs) {
      const batchText = logs.map(l => l.content).join('\n');
      const logIds = logs.map(l => l.id);
      const summaryPrompt = `Resuma as seguintes anotações do projeto #${projectTag} focando em decisões técnicas e regras de negócio:\n${batchText}`;
      const summary = await callOpenRouter(summaryPrompt, "google/gemini-2.0-flash-001");
      const embedding = await generateEmbedding(summary);
      if (embedding) {
        await supabase.from('memories').insert({ project_tag: projectTag, summary: summary, embedding: embedding });
        await supabase.from('brain').update({ metadata: { consolidated: true } }).in('id', logIds);
      }
    }
  } catch (e) { console.error("Falha ao consolidar memória:", e); }
}
