...
export async function GET(req: NextRequest) {
  ...
  const userId = await getUserFromToken(token); // bigint como number
  ...
  // 1. Confirma que o lembrete pertence ao usuário
  ...
  // 2. Busca o auth_user_id do dono (para cruzar com relationships)
  ...
  if (!userRow?.auth_user_id) {
    return NextResponse.json({ error: 'Usuário sem auth_user_id' }, { status: 404 });
  }

  // 3. Vínculos ativos com agenda_enabled = true
  ...
  // 4. Resolve UUIDs dos parceiros para bigint_id
  ...
  // 6. Monta opções no mesmo formato que ShareEventModal espera
  const options = agendaRels.map(rel => {
    ...
    return {
      ...
      contact_name:
        partner.preferred_name || partner.nickname || partner.name ||
        rel.contact_name || 'Contato',
