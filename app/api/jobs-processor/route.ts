...
export async function POST(req: NextRequest) {
  ...
  const {
    msg_id, userId, message, reply,
    contexts, emotional, authorName,
    assistantName, sessionId, modelRoute,
  } = payload;
  ...
  try {
    ...
    if (!existingBrain) {
      ...
      await supabase.from('brain').insert([{
        ...
        embedding,
        category:        'info',
        // emotional_score persiste no campo de raiz para calcularPesoEmocional()
        emotional_score: emotional?.score ?? 0,
        metadata: {
          ai_reply:        reply,
          msg_id,
          // redundante mas mantém compatibilidade com leitores legados
          emotional_score: emotional?.score ?? 0,
          recovered:       true,
        },
      }]);
    ...
    // ── 2. Tasks paralelas ───────────────────────────────────────────────────
    const tasks = [
      runIdempotentTask('unified_extractor', msg_id, () =>
        runUnifiedExtractor(userId, authorName, message, reply)
      ),
      runIdempotentTask('profile_extractor', msg_id, () =>
        extractProfileFromConversation(parseInt(userId), message, reply)
      ),
    ];
    ...
    if (Math.random() <= 0.30) {
      tasks.push(runIdempotentTask('critic', msg_id, async () => {
        const criticPrompt =
          `Você é um avaliador interno de qualidade de um assistente de IA pessoal chamado ${assistantName}.\n` +
        ...
        if (criticRes?.content) {
          ...
          await redis.set(
            `critic_history_${userId}`,
            [...history, criticScore].slice(-10),
            { ex: 86400 }
          );
        }
      }));
    ...
    // PATTERN PROMOTER (10%)
    if (Math.random() <= 0.10) {
      tasks.push(runIdempotentTask('pattern_promoter', msg_id, () =>
        promotePatternToPrinciple(parseInt(userId), authorName, assistantName)
      ));
    }
