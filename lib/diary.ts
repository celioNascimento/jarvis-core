...
export async function extractDiary(
  ...
  const prompt = `Analise a mensagem e extraia dados de diário. Retorne JSON (null se não houver):
{ "eh_diario": boolean, "content": string|null, "mood": number|null, "energy": number|null, "gratitude": string[], "intention": string|null, "reflection": string|null }`;
  ...
  try {
    ...
    if (data.mood !== undefined && data.mood !== null) payload.mood = data.mood;
    if (data.energy !== undefined && data.energy !== null) payload.energy = data.energy;
    if (data.intention) payload.intention = data.intention;
    if (data.reflection) payload.reflection = data.reflection;

    if (Array.isArray(data.gratitude) && data.gratitude.length > 0) {
...
export async function updateGoalProgress(
  ...
  try {
    ...
    // Regra 2: Invalidação aplicada aqui também
