...
// Dispatcher V11.2.0 — Adiciona esportes_consultar_jogos_por_time
...
const TOOL_ROUTER: Record<string, ToolHandler> = {
  ...
  'email_listar_recentes': async (p) => {
    ...
    const [g, m] = await Promise.all([
      Google.executeGoogleListarEmails(p as any),
      Microsoft.executeMicrosoftListarEmails(p as any),
    ]);
    return `${g}\n\n${m}`;
  },
  'email_excluir': async (p) =>
    p.provedor === 'google' ? await Google.executeGoogleExcluirEmail(p as any) : 'N/A',
  ...
  'esportes_consultar_jogos_por_time':  (p) => Esportes.executeConsultarJogosPorTime(p as any),
  'esportes_consultar_tabela':          (p) => Esportes.executeConsultarTabela(p as any),

  // Web, Clima, Relacionamentos
...
function preFlightFilter(toolCalls: ToolCall[]): ToolCall[] {
  ...
  const hasUpdate = unique.some(
    tc => tc.function.name.includes('_atualizar') || tc.function.name.includes('_salvar'),
  );
